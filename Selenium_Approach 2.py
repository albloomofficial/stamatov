...
with open('Srcs_Burney_{}_slave3.csv'.format(datetime.datetime.now().strftime('%y-%m-%d')), 'a') as f:
        writer = csv.writer(f)
        writer.writerows(final_data)


#Create Selenium websriver with a PhantomJS
def initialize_driver(page, driver_name="PhantomJS"):
    ...
    time.sleep(5)
    # driver.find_element_by_xpath('//*[@id="fascimileForm"]/table/tbody/tr/td[1]/a[2]').click()

    #set up wile loop
    # page_selector = driver.find_element_by_xpath('//*[@id="currPosTop"]')
    # page_selector.setAttribute('value', page)
    page_selector = driver.find_element_by_xpath('//*[@id="currPosTop"]')
    page_selector.click()
    page_selector.clear()
...
# driver = initialize_driver(1, driver_name="Chrome")
...
def run_thru_pages(page_range, increment):
    ...
    while page < page_range + increment:
        ...
        try:
            ...
            final_data = zip(*[author,location, date, sources, src_links, gale_number])
            with open('Srcs_Burney_02_10_2018_slave3.csv', 'a') as f:
                writer = csv.writer(f)
                writer.writerows(final_data)
        except:
            time.sleep(60*60)
...
if __name__ == "__main__":
    slave_names = ["driver{}".format(i+1) for i in range(multiprocessing.cpu_count()-4)]
    pages = 56500
    increment = math.ceil(pages / (multiprocessing.cpu_count()-4))
    print(increment)
    pool = multiprocessing.Pool()
    for i in range(multiprocessing.cpu_count()-4):
        url_range = increment * i
        # slave_name = slave_names[i]
        csv_file_name = "Srcs_Burney_02_10_2018_slave3.csv"
        # pool.apply_async(run_thru_pages(url_range, increment))
        new_process = multiprocessing.Process(name=slave_names[i], target=run_thru_pages, args = (url_range, increment))
        new_process.start()
    pool.close()
    pool.join()
    # driver = initialize_driver(10000, driver_name="Chrome")
